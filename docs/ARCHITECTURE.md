...
| **R3 持久化** | 写记忆、提交用例集 | **人工审批**（human-in-the-loop） | ⬜ R5 |
...
| R1 沙箱写（生成用例 → 编译 → 跑） | ⬜ | R4 |
| R3 持久化 + human-in-the-loop 审批 | ⬜ | R5 |
| 真 embedding / 图谱入融合 / rerank | ⬜ 复用 platform 现状（两路 RRF） | R4 |
| 情景/程序性记忆 + 离线巩固 | ⬜ 轨迹已落盘，检索与巩固未做 | R5 |
| 评测任务集 / A-B 门禁 | ⬜ | R6 |
| `nolib/` 手写最小 loop 对比 | ⬜ | R7 |
