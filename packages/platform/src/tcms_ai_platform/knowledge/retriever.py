...
_DOMAIN_TERMS: dict[str, tuple[str, ...]] = {
    ...
    # 注意区分**系统词**与**位置/现象词**：`客室` 是位置词（归 hvac，因为"客室温度"要用它），
    # 于是"客室灯带频闪"这类查询曾整条被路由进 hvac 域，light 域的症状根本不在候选里——
    # diagnoser 早就记录过这个坑并在自己那条链上绕开了域路由，检索通道这里补齐词表。
    # 补的是**照明专属**词：`灯带`（部件）、`频闪`（灯特有的闪烁方式）。
    # **不要**把 `闪烁` 放进来——它是跨域现象词（灯会闪、仪表盘会闪、PIS 屏会闪），
    # 收进来会让"仪表盘闪烁但无故障码"被路由进 light 域，反而把原本命中的
    # symptom:dashboard_flicker 隔离在候选之外（golden 门禁当场抓到了这条回退）。
    "light": ("照明", "应急灯", "灯光", "灯带", "频闪", "light", "lamp"),
