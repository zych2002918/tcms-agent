...
[![pytest](https://img.shields.io/badge/pytest-374%20passed-2dd4a0)](#测试--门禁)
...
- **GraphRAG 风格混合检索**：BM25 词法 + 向量双路等权 RRF 召回，**图谱通道作为纯补充路**并入融合——
  它只交出文本两路未召回的结构相邻文档，因而只可能增加召回、不会稀释排序
  （早期版本的图谱只做域路由与证据富化、并不参与融合，README 却写成"三通道"，已改正）；
  每条命中都带 `channels` 标注，可逐条核对是被哪几路召回的。
  默认**字符级确定性通道**（离线、可复现），真语义近义为**可选增强**（`TCMS_EMBEDDER=api` 接入 OpenAI 兼容 embedding，无 key/失败自动降级，绝不冒充语义）；
- **图谱通道的真实价值点（实测）**：症状式查询（如「仪表盘闪烁」，无实体名、文本重合低）能拿到
  仅图谱召回的补充资产（`system:` / `function:` / `fault:`）；而带实体名的查询（如「车门故障」）
  文本两路已足够，图谱补不出新东西。这两种情况的差异是实测结论，不是设计预期；
...
| 门禁 | 结果 | 说明 |
|---|---|---|
| `pytest -q` | **374 passed + 1 skipped** | 检索 28 golden、诊断 8 golden、30 条对抗集、图谱通道契约、上游路径护栏、多轮/澄清/MCP/function-calling 契约、**设计 token 对比度门禁**、**图谱骨架不变量（无孤立点 / 无悬空边 / 可复现）**… |
...
```bash
python -m pytest tests -q                 # 374 passed + 1 skipped
