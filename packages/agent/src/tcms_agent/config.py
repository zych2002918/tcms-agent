...
class AgentConfig:
    ...
    """送进模型的**上下文总字符预算**（超出则压缩较早的工具结果，见 `nodes._apply_context_budget`）。

    为什么需要：单条工具结果已被 `MAX_RESULT_CHARS` 截到 4000 字符，但 12 步 × 每步最多
    两条结果，最坏能堆到近 10 万字符——这还没算系统提示与记忆块。长任务因此会悄悄逼近
    模型窗口，而"悄悄"正是这里最糟的部分：超窗的报错发生在模型侧，轨迹里什么都看不出来。
    改成显式预算后，每次压缩都会写进轨迹（`event=context_budget`）。

    48000 这个默认值的来源：最坏情况的约一半，留出系统提示 + 记忆 + 结论的余量。
    """
