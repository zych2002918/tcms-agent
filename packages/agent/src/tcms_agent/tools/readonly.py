...
def _bind(
    ...
    *,
    ...
    实测（28 条检索 golden，2026-09-21 由 14 条扩容）：min_at 达标 27/28，**期望项进
    top-1 由 23/28 升到 25/28**；扩容同时暴露 3 条中文描述性查询被排差（`id_hit` 要求
    查询词出现在英文 doc_id 中，中文查询天然拿不到这一分），已登记为待修工单。
    `rerank=False` 时保持原序，供 A/B 对照使用。
    """
