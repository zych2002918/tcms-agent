...
仓里有 `nolib/` —— 一份 **175 有效行、零编排框架**的最小 loop，与本版**共用同一批节点、
同一套工具、同一个模型**，只替换编排层。用途是把"框架到底提供了什么"变成可测量的对照：

```bash
uv run tcms-agent nolib --verify
```

实测：**同集 11 条任务逐条一致**（91% / 91%，平均步数 4.82 / 4.82）。
结论：**框架不参与决策，因此不改变任务结果**；它买到的是状态落盘、断点续跑、
人工审批中断、轨迹回放、流式观测——这些手写要更多代码且极易在
"被中断的节点会从头重跑、副作用必须放在 interrupt 之后"这类细节上出错。

## 测试

```bash
cd packages/agent
pytest -q      # 192 passed + 2 skipped：无 API key，全部离线可复现
```
