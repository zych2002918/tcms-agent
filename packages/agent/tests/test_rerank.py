...
#: 共同点是**中文描述性查询**（融合后都排第 1，重排后被挤下去）。
#: 曾怀疑根因是 id_hit 对中文失效；2026-09-21 把它的权重转给 query_overlap 后
#: **实测指标一字未变**——那个怀疑不成立，真正主导的是 channel_consensus(0.45)。
#: 所以这 3 条是**仍未解决**的工单，不是"已定位待修"。
